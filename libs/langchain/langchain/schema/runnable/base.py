    ...
    def invoke(self, input: Input, config: Optional[RunnableConfig] = None) -> Output:
        ...
        try:
            for i, step in enumerate(self.steps):
                if isinstance(step, StructuredQueryOutputParser):
                    if isinstance(input, str):
                        #correct the llm text
                        input = correct_llm_output(llm_text= input)
                    elif isinstance(input, LLMResult):
                        #correct the llm output
                        input = correct_llm_output_cls(llm_result= input)
...
def correct_llm_output_cls(llm_result ):
    from langchain.schema import Generation, LLMResult
    generations = llm_result.generations
    corrected_generations = []
    for generation in generations:
        gtext = generation.text
        
        # Try to find JSON string within triple backticks
        match = re.search(r"```(json)?(.*)```", gtext, re.DOTALL)
        correct_text = ''
        if match:
            if (gtext.endswith('\n```') and '\n\n\n<<' not in gtext):
                correct_text = gtext
            elif('\n\n\n<<' in gtext):
                # correct_text = gtext.rsplit('\n\n\n<<', 1)[0]
                correct_text = gtext.split('\n\n\n<<', 1)[0]
        if correct_text:
            generation.text = correct_text            
        corrected_generations.append(generation)
    
    llm_result.generations = corrected_generations
    
    return llm_result

def correct_llm_output(llm_text:str ):   
    
    # Try to find JSON string within triple backticks
    match = re.search(r"```(json)?(.*)```", llm_text, re.DOTALL)
    correct_text = ''
    if match:
        if (llm_text.endswith('\n```') and '\n\n\n<<' not in llm_text):
            correct_text = llm_text
        elif('\n\n\n<<' in llm_text):
            # correct_text = llm_text.rsplit('\n\n\n<<', 1)[0]
            correct_text = llm_text.split('\n\n\n<<', 1)[0]
    if correct_text:
        llm_text = correct_text
